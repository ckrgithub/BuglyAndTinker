...
public class MainActivity extends BaseActivity implements View.OnClickListener, OnInstallApkListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setOnClickListener(R.id.btnPatch);
			...
			if (canInstall) {
				...
				if (upgradeInfo != null) {
					...
					if (downloadStatus == COMPLETE) {
						ApkUtil.installApk(ApkUtil.getApkPath(upgradeInfo.apkUrl, context), this, false);
					}
				}
			}
		}
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.btnPatch:
				...
				boolean tinkerEnabled = tinker.isTinkerEnabled();
				boolean tinkerLoaded = tinker.isTinkerLoaded();
				boolean enableHotfix = Beta.enableHotfix;
				Logd(TAG, "onClick: tinkerEnabled:" + tinkerEnabled + ",tinkerLoaded:" + tinkerLoaded + ",enableHotfix:" + enableHotfix);
				String newTinkerId = TinkerManager.getNewTinkerId();
				String tinkerId = TinkerManager.getTinkerId();
				Logd(TAG, "onClick: newTinkerId:" + newTinkerId + ",tinkerId:" + tinkerId);
				if (tinkerEnabled && !tinkerLoaded && enableHotfix) {
					Beta.downloadPatch();
				}
				break;
		}
